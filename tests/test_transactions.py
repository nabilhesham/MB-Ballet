...
def clients(conn):
    return conn.execute("SELECT COUNT(*) n FROM clients").fetchone()["n"]


def add(conn, name):
    return _ins(conn, "clients", name_en=name, created_at=db.now(), active=1)


# ---------------------------------------------------------------- basics

def test_a_block_that_finishes_is_committed(conn):
    with db.tx(conn):
        add(conn, "Kept")
    assert clients(conn) == 1
    assert not conn.in_transaction


def test_a_block_that_raises_is_rolled_back(conn):
    with pytest.raises(ValueError):
        with db.tx(conn):
            add(conn, "Doomed")
            raise ValueError("something went wrong halfway")
    assert clients(conn) == 0
    assert not conn.in_transaction


def test_every_write_in_a_failed_block_goes_back(conn):
...
def test_a_write_outside_a_block_autocommits(conn):
...
def test_an_inner_block_does_not_commit_early(conn):
    """
...
def test_a_failure_after_an_inner_block_rolls_the_inner_one_back_too(conn):
    """
...
def test_nesting_survives_three_levels(conn):
    with db.tx(conn):
        with db.tx(conn):
            with db.tx(conn):
                add(conn, "Deep")
...
def test_the_connection_is_reusable_after_a_rollback(conn):
    with pytest.raises(ValueError):
        with db.tx(conn):
            add(conn, "Doomed")
            raise ValueError
...
def test_a_block_takes_the_write_lock_at_the_top(conn):
    """
...
def a_session_today_they_are_not_booked_into(a):
    ...
    """
    return add_session(a.conn, a.flex, a.bea, db.now() + 2 * 3600, 1.0,
                       status="scheduled")
...
def test_a_failed_swap_leaves_the_booking_where_it_was(academy, monkeypatch):
    ...
    events_before = conn.execute("SELECT COUNT(*) n FROM access_events").fetchone()["n"]

    monkeypatch.setattr(access, "check_in",
                        lambda *args, **kw: (_ for _ in ()).throw(RuntimeError("crash")))
    with pytest.raises(RuntimeError):
        access.swap_and_check_in(conn, a.dual, give_up, target)
    ...
    assert conn.execute("SELECT COUNT(*) n FROM access_events").fetchone()["n"] \
...
def test_a_successful_swap_moves_the_booking_and_checks_in(academy):
    ...
    r = access.swap_and_check_in(conn, a.dual, give_up, target)
...
def test_the_sweep_and_the_freezes_it_lifts_are_one_transaction(academy, monkeypatch):
    ...
    a = academy
    conn = a.conn
    access.freeze_plan(conn, a.dual_ballet_plan,
                       from_date=(date.today() - timedelta(days=8)).isoformat(),
                       until=(date.today() - timedelta(days=1)).isoformat())
    assert access.plan_state(conn, a.dual_ballet_plan)["frozen"] is True
...
def test_refresh_expiry_is_committed_by_whoever_opened_the_block(academy):
    ...
    a = academy
    conn = a.conn
    before = access.plan_state(conn, a.dual_ballet_plan)["expires_on"]
    latest = conn.execute(
    ...
    access.unbook(conn, a.dual, latest)

    assert not conn.in_transaction, "unbook closed its own block"
    after = access.plan_state(conn, a.dual_ballet_plan)["expires_on"]
