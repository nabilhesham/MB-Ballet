...
def _ins(repo, table: str, **cols) -> int:
    """
    Insert through the repository, not through SQL.

    The fixture has to build the same academy on whichever backend is under
    test, so it may not know what SQL is. Keyword arguments only for
...
def _card(repo, client_id: int, class_id: int) -> str:
    ...
    _ins(repo, "credentials", client_id=client_id, class_id=class_id,
         token=token, kind="card", issued_at=db.now())
    return token


def _plan(repo, client_id, class_id, name, total, *, price=None,
          starts_on=None, paid_on=None) -> int:
    today = date.today()
    return _ins(repo, "subscriptions",
...
def _fill(repo, client_id, sub_id, session_ids):
    """
...
def build_academy(repo) -> SimpleNamespace:
    """
    ...
    with repo.begin():
        ...
        a.dual_ballet_plan = _plan(repo, a.dual, a.ballet, "12 sessions", 12,
                                   price=4100.0, paid_on=today.isoformat())
        _fill(repo, a.dual, a.dual_ballet_plan,
        ...
        a.dual_flex_plan = _plan(repo, a.dual, a.flex, "8 sessions", 8, price=2000.0)
        _fill(repo, a.dual, a.dual_flex_plan, (past_f + future_f)[:8])

        a.solo_ballet_plan = _plan(repo, a.solo_ballet, a.ballet, "12 sessions", 12,
                                   price=4100.0)
        _fill(repo, a.solo_ballet, a.solo_ballet_plan, (past_b + future_b)[:12])
        ...
        a.solo_flex_plan = _plan(repo, a.solo_flex, a.flex, "4 sessions", 4,
                                 price=None)
        _fill(repo, a.solo_flex, a.solo_flex_plan, future_f[:4])

        a.lapsed_plan = _plan(repo, a.lapsed, a.ballet, "4 sessions", 4, price=900.0,
                              starts_on=today - timedelta(days=120))
        _fill(repo, a.lapsed, a.lapsed_plan, past_b[:4])

        # -------------------------------------------------- cards
        # One per class, which is what makes "the card decides the class" mean
        # anything for the dual client.
        a.dual_ballet_card = _card(repo, a.dual, a.ballet)
        a.dual_flex_card = _card(repo, a.dual, a.flex)
        a.solo_ballet_card = _card(repo, a.solo_ballet, a.ballet)
