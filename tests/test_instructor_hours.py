...
def test_taught_hours_reports_scheduled_and_adjustment_apart_from_their_sum(academy):
    ...
    t = access.taught_hours(academy.conn, academy.ana, a, b)
...
def test_an_adjustment_moves_the_taught_total_to_what_was_asked_for(academy):
    ...
    before = access.taught_hours(conn, academy.ana, day, day)["hours"]

    result = access.adjust_taught_hours(conn, academy.ana, day, before + 1.5,
...
def test_an_adjustment_is_a_dated_delta_not_a_rewrite(academy):
    ...
    access.adjust_taught_hours(conn, academy.ana, day, 99.0)
...
def test_a_correction_never_reaches_the_salary_sheet_figure(academy):
    ...
    logged_before = access.logged_hours(conn, academy.ana, a, b)

    access.adjust_taught_hours(conn, academy.ana, day, 99.0)

    logged_after = access.logged_hours(conn, academy.ana, a, b)
...
def test_days_worked_counts_only_real_salary_sheet_rows(academy):
    ...
    conn = academy.conn
    a, b = (date.today() - timedelta(days=30)).isoformat(), date.today().isoformat()
    days_before = access.logged_hours(conn, academy.ana, a, b)["days"]

    access.adjust_taught_hours(
        conn, academy.ana, (date.today() - timedelta(days=90)).isoformat(), 5.0)

    assert access.logged_hours(conn, academy.ana, a, b)["days"] == days_before
...
def test_a_wider_range_picks_up_a_days_correction_by_summing(academy):
    ...
    day = (date.today() - timedelta(days=1)).isoformat()
    access.adjust_taught_hours(
        conn, academy.ana, day,
        access.taught_hours(conn, academy.ana, day, day)["hours"] + 2.0)

    a, b = a_week_around_today()
    assert access.taught_hours(conn, academy.ana, a, b)["adjustment"] == 2.0
