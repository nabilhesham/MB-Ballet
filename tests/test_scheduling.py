...
def empty(conn):
    ...
    ins = _ins(conn, "instructors", name="Ana", hourly_rate=100.0, active=1)
    a = _ins(conn, "classes", name="Ballet", colour="#87438E",
             duration_hours=1.5, instructor_id=ins, active=1)
    b = _ins(conn, "classes", name="Flex", colour="#EAAECA",
             duration_hours=1.0, instructor_id=ins, active=1)
    conn.commit()
    return type("E", (), {"conn": conn, "ins": ins, "ballet": a, "flex": b})
...
def add(e, class_id, starts_at, hours=1.0, status="scheduled"):
    sid = add_session(e.conn, class_id, e.ins, starts_at, hours, status=status)
    e.conn.commit()
    return sid
...
def test_an_empty_timetable_has_no_conflict(empty):
    assert access.slot_conflict(empty.conn, at(1, 18), 1.5) is None


def test_an_exact_overlap_is_refused(empty):
    add(empty, empty.ballet, at(1, 18), 1.5)
    clash = access.slot_conflict(empty.conn, at(1, 18), 1.5)
...
def test_a_session_does_not_clash_with_itself_when_excluded(empty):
    ...
    assert access.slot_conflict(empty.conn, at(1, 18), 1.5) is not None
    assert access.slot_conflict(empty.conn, at(1, 18), 1.5, exclude_id=sid) is None
...
def test_the_rule_applies_to_the_past_too(empty):
    ...
    assert access.slot_conflict(empty.conn, at(-7, 18), 1.5) is not None
...
def test_the_earliest_clashing_session_is_the_one_reported(empty):
    ...
    clash = access.slot_conflict(empty.conn, at(1, 18), 3.0)
...
def test_a_fractional_duration_is_respected(empty):
    ...
    add(empty, empty.ballet, at(1, 18), 1.5)          # 18:00-19:30
    assert access.slot_conflict(empty.conn, at(1, 19, 0), 0.5) is not None
    assert access.slot_conflict(empty.conn, at(1, 19, 30), 0.5) is None
...
def test_a_session_that_has_ended_becomes_completed(empty):
    ...
    access.settle_past_sessions(empty.conn)
    assert empty.conn.execute("SELECT status FROM sessions WHERE id=?",
...
def test_a_booking_on_a_cancelled_session_is_not_swept_absent(empty):
    conn = empty.conn
    sid = add(empty, empty.ballet, db.now() - 4 * 3600, 1.5, status="cancelled")
    cid = _ins(conn, "clients", name_en="X", created_at=db.now(), active=1)
    _ins(conn, "bookings", client_id=cid, session_id=sid,
         status="booked", created_at=db.now())
    conn.commit()
    access.settle_past_sessions(conn)
    assert conn.execute("SELECT status FROM bookings WHERE session_id=?",
...
def test_the_sweep_returns_how_many_bookings_it_settled(empty):
    ...
    for offset in (4, 8):
        ...
        _ins(conn, "bookings", client_id=cid, session_id=sid,
             status="booked", created_at=db.now())
    conn.commit()
    assert access.settle_past_sessions(conn) == 2
...
def test_the_sweep_is_idempotent(empty):
    ...
    _ins(conn, "bookings", client_id=cid, session_id=sid,
         status="booked", created_at=db.now())
    conn.commit()
    assert access.settle_past_sessions(conn) == 1
    assert access.settle_past_sessions(conn) == 0
...
def test_migrate_repairs_a_session_whose_end_is_missing(empty):
    ...
    conn = empty.conn
    sid = add(empty, empty.ballet, at(1, 18), 1.5)
    conn.execute("UPDATE sessions SET ends_at=NULL WHERE id=?", (sid,))
    conn.commit()
    assert access.slot_conflict(conn, at(1, 18), 1.5) is None, "precondition: invisible"

    db.migrate(conn)

    assert access.slot_conflict(conn, at(1, 18), 1.5) is not None
    assert conn.execute("SELECT ends_at FROM sessions WHERE id=?",
