...
[ -n "$PY" ] || die "Python 3 is needed to BUILD this — the finished program won't need it. Install it from python.org, then run this again."
ok "Python $("$PY" -c 'import sys; print(sys.version.split()[0])')"

step "[1/4] Installing the build tool…"
# Upgrading pip itself is best-effort: some Python installs (Homebrew's in
# particular) manage pip themselves and refuse to let pip upgrade itself —
# that's not a reason to stop, since the pip already there still works fine.
