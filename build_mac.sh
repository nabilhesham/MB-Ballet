#!/usr/bin/env bash
# ===========================================================================
#  Build a standalone MB Ballet Academy binary — macOS
#
#  Run this ONCE, on a Mac that has Python, when you want to hand the
#  reception laptop something with nothing to install at all. It produces
...
#
#  This build is unsigned (no Apple Developer certificate involved). If the
#  reception laptop is a different Mac than the one that built it, macOS
#  will flag the copied file as being from an unidentified developer the
#  first time it runs — right-click (or Control-click) the file and choose
#  Open, then confirm, instead of double-clicking. After that first time it
#  opens normally.
# ===========================================================================
...
#
# Not tidiness: a Homebrew Python refuses `pip install` outright (PEP 668,
# "externally-managed-environment"), so the previous version of this step
# could not work with the very interpreter the version check now steers
# people towards. Installing into the machine's own Python was never a good
# idea anyway — the same reasoning START.bat gives for .venv-windows.
#
# PyInstaller bundles what it can see, so the app's dependencies go in here
# too, not just the build tool.
...
if [ -x "$BUILD_VENV/bin/python" ] && "$BUILD_VENV/bin/python" -c "" 2>/dev/null; then
  ...
  # START.bat makes.
  rm -rf "$BUILD_VENV"
  "$PY" -m venv "$BUILD_VENV" || die "Could not create the build environment in $BUILD_VENV"
fi
